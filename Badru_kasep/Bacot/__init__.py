...
class gadag_user:
	...
	def followers(self,link,what=False):
		try:
			...
			for __ in _:
				...
				aap_gans.append(___.group(1)+"(Aap Gans)"+__[1] if ___ is not None else __[0]+"(Dru_Gans)"+__[1])
				print(f"\r * mengumpulkan {len(Dru_Gana)} user, ctrl+c stop",end="")
	...
	def fl(self,link,what=False):
		try:
			...
			for __ in _:
				aap_gans.append(re.search("id=(\d*)" if "profile.php" in __ [0] else "(.*?)\?",__[0]).group(1)+"(Dru Gans)"+__[1])
	...
	def grup(self,link,why,what=False):
		try:
			...
			for __ in _:
				...
				print(f"\r * mengumpulkan {len(Dru_Gans)} user, ctrl+c stop",end="")
			if "Lihat Selengkapnya" in link:
